...
NEW_HINA_DATA_CLASS(
		...
		CubbyFlow::SPHSystemData3Ptr InnerPtr;
				...
				NEW_GETSET_PARAMETER(EosExponent, GETSET_DATA_FUNCS_F)
				NEW_GETSET_PARAMETER(NegativePressureScale, GETSET_DATA_FUNCS_F)
				NEW_GETSET_PARAMETER(ViscosityCoefficient, GETSET_DATA_FUNCS_F)
				NEW_GETSET_PARAMETER(PseudoViscosityCoefficient, GETSET_DATA_FUNCS_F)
				NEW_GETSET_PARAMETER(SpeedOfSound, GETSET_DATA_FUNCS_F)
				NEW_GETSET_PARAMETER(TimeStepLimitScale, GETSET_DATA_FUNCS_F)
				NEW_GETSET_PARAMETER(MaxDensityErrorRatio, GETSET_DATA_FUNCS_F)
				NEW_GETSET_PARAMETER(MaxNumberOfIterations, GETSET_DATA_FUNCS_F)
				NEW_GETSET_PARAMETER(ActivateFluidDomainCollider, GETSET_DATA_FUNCS_B)
				NEW_GETSET_PARAMETER(FluidDataType, GETSET_DATA_FUNCS_I)

				size_t substep_num = 0; // update every frame
				void runtime_init_handles(GU_Detail &gdp); // Call Inside Lock
				double calculate_dynamic_dt();
